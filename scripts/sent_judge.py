# -*- coding: utf-8 -*-
"""문장 대장의 한 구간을 판정한다. **`15` 4항의 절차를 그대로 돌린다.**
...
왜 이 도구가 있나
  ...
  `15` 4항는 절차를 글로 적어 두었는데 **그 절차를 돌리는 도구가 없었다.**
  그래서 검수하는 쪽이 임시 스크립트를 따로 만들었고, 기능 갈래를 4-1항의
...
무엇을 하나 (`15` 4-1항 - 4-4항)
  ...
  1 문장의 **기능**을 4-1항의 열 갈래 중 하나로 정한다. 못 정하면 비워 둔다
...
# ── 기능 열 갈래. `15` 4-1항의 표 그대로다. **여기에 없는 이름은 안 쓴다** ──
...
def function_of(s):
    """4-1항의 열 갈래 중 하나. **못 정하면 None을 낸다.**
...
def pick_evidence(idx, fn, ours):
    """같은 기능의 게재작 문장 하나. 앞뒤 한 문장을 함께 붙인다(4-2항)."""
    want = content(ours)
    best = None
...
def main():
    ...
    for i, c in enumerate(rows, 1):
        ...
        # 기능의 선례와 기계 검사뿐이다. `15` 4-3항이 요구하는 여섯 항목 중
    ...
    if left:
        ...
        print("")
        print("- 사람이 4-1항의 열 갈래 중 하나로 기능을 정하고, 그 기능의"
...
def verify(rows, a, b, docs):
    ...
    for i, no, why in bad[:20]:
        print("    - [%d행 %s] %s" % (i, no, why))
    if wrong_fn:
        print("- **기능이 4-1항의 열 갈래 밖인 행 %d개.** 지어낸 갈래를 쓰면"
