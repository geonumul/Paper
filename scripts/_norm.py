...
# 통화만 가린다. 수식의 닫는 달러를 통화로 오인하면 안 된다.
# `$^{***}p<0.001$.` 의 닫는 달러가 뒤의 마침표 때문에 통화로 잡혀,
# 남은 달러가 다음 수식과 짝지어 그 사이 문장을 삼킨 적이 있다.
#   ① 나라 표시가 붙은 것: US$ 4 million
#   ② 낱말 경계에서 시작해 숫자가 뒤따르는 것: ($4,000)
_CURRENCY = re.compile(r"(?:US|A|C|NZ|HK|S)\$(?=\s?\d)"
                       r"|(?<![\w}$])\$(?=\s?\d)")
_MASK = ""
